<template>
  <v-row>
    <v-col cols="12" v-if="smAndDown" class="text-wrap text-h4 text-black mx-sm-6">
      <v-skeleton-loader v-if="rootStore.loading || rootStore.news.loading"
        :type="['heading', 'heading'][['xs', 'sm'].indexOf(name || 'sm')]"></v-skeleton-loader>
      <template v-else>
        <v-chip class="mb-4">{{
    ...
    <v-col cols="12" md="4" class="pb-0">
      <v-skeleton-loader v-if="rootStore.loading || rootStore.news.loading" height="100%"
        type="image"></v-skeleton-loader>

      <div v-else class="mx-sm-6">
        <MiscAtomsImageContainer cover :loading="rootStore.news.loading" :src="item.image" :ratio="1 / 1" />
      </div>
    </v-col>

    <v-col cols="12" md="8" class="pl-0 pb-0 d-flex flex-column justify-md-end">
      <v-skeleton-loader v-if="rootStore.loading || rootStore.news.loading" :type="[
          'text, chip@4',
          'text, chip@4',
          'heading, text',
          'heading, text',
      ...
      <div v-else class="mx-4 mx-md-0 justify-md-end">
        ...
        <div class="d-flex flex-column flex-md-row align-md-center mt-6 mx-sm-4 mx-md-6">
          <!-- <MiscAtomsDateStamp
          ...
          <div class="ml-md-n6">
            ...
            <div class="" v-if="smAndDown">
              <MiscMoleculesChipContainer :items="item.tags" class="mt-4"></MiscMoleculesChipContainer>
            </div>
          </div>
        </div>
      </div>
  ...
  <v-row class="mt-12">
    <v-col cols="12" md="4" :order="mdAndUp ? 'first' : 'last'" class="pt-0">
      <v-skeleton-loader v-if="rootStore.loading || rootStore.news.loading" :type="['chip@2', 'chip@3', 'chip@4', 'chip@5'][
    ...
    <v-col cols="12" md="8" class="pl-0 pt-0">
      <v-skeleton-loader v-if="rootStore.loading || rootStore.news.loading" :type="['text@50', 'text@50', 'text@50', 'text@50', 'text@50', 'text@50'][
        ['xs', 'sm', 'md', 'lg', 'xl', 'xxl'].indexOf(name || 'md')
        ]
        "></v-skeleton-loader>
      <template v-else>
  ...
  <MiscAtomsSlidingCarousel :items="item.gallery" type="MiscAtomsImage" ref="MiscAtomsImage" key="MiscAtomsImage"
    :loading="false">
  ...
  <v-row>
    ...
    <v-col cols="12" md="4">
      <MiscMoleculesRelatedItems type="project" :items="item.relatedProjects"></MiscMoleculesRelatedItems>
    </v-col>
    <v-col cols="12" md="4">
